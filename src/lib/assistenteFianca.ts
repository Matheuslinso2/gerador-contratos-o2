...
0. LEITURA BRUTA ("leitura_bruta") — SEMPRE preencha primeiro, antes de qualquer outro campo. Se o panorama vier como TEXTO, escreva 1 frase curta confirmando os dados recebidos. Se vier como IMAGEM OU PDF, faça uma transcrição cuidadosa, seguradora por seguradora, ANTES de decidir qualquer taxa ou multiplicador (um PDF pode ter várias páginas — transcreva página por página se precisar):
   ...
   c) dentro de cada grade, liste os cabeçalhos de coluna na ordem exata em que aparecem, da esquerda para a direita, e diga célula por célula se está vazia/cinza ou tem um valor preenchido;
   d) BUG CONHECIDO NESSES PAINÉIS: alguns sistemas de origem imprimem um texto de multiplicador (ex: "30X") colado ao lado do valor de cada célula preenchida, e esse texto pode estar ERRADO/travado (o sistema sempre escreve o mesmo multiplicador do lado do valor, independente da coluna real onde o valor está). NUNCA use esse texto colado ao valor para decidir o multiplicador. A ÚNICA forma confiável é contar a POSIÇÃO da célula preenchida a partir do cabeçalho da coluna correspondente (1ª coluna da esquerda = primeiro multiplicador do cabeçalho, e assim por diante) — reporte essa posição contada, não o texto impresso ao lado do valor, e diga explicitamente na leitura_bruta que fez essa contagem (ex: "valor de R$X está na 1ª coluna da grade do Básico, ou seja, multiplicador 18x, mesmo que o texto ao lado do valor diga outro número").
   e) se a posição da célula preenchida não estiver 100% legível (colunas estreitas, imagem comprimida ou baixa qualidade), diga isso explicitamente aqui em vez de adivinhar.
