...
0. LEITURA BRUTA ("leitura_bruta") — SEMPRE preencha primeiro, antes de qualquer outro campo. Se o panorama vier como TEXTO, escreva 1 frase curta confirmando os dados recebidos. Se vier como IMAGEM OU PDF, faça uma transcrição cuidadosa, seguradora por seguradora, ANTES de decidir qualquer taxa ou multiplicador (um PDF pode ter várias páginas — transcreva página por página se precisar):
   ...
   c) dentro de cada grade, liste os cabeçalhos de coluna na ordem exata em que aparecem, da esquerda para a direita, e diga célula por célula se está vazia/cinza, tem só um valor em R$, ou tem uma marcação (X, ✓, ou similar) indicando aprovação — quando uma célula tiver tanto o valor quanto uma marcação de X/check, é essa marcação que confirma qual LMI foi efetivamente aprovado para aquele plano, não presuma pelo valor sozinho se houver mais de uma célula preenchida na mesma grade;
   d) se a correspondência entre um valor/marcação e sua coluna não estiver 100% legível (colunas estreitas, imagem comprimida ou baixa qualidade), diga isso explicitamente aqui ("valor de R$X visível, mas a coluna exata entre 18x/24x/30x não está nítida") em vez de adivinhar.
