...
0. LEITURA BRUTA ("leitura_bruta") — SEMPRE preencha primeiro, antes de qualquer outro campo. Se o panorama vier como TEXTO, escreva 1 frase curta confirmando os dados recebidos. Se vier como IMAGEM OU PDF, faça uma transcrição cuidadosa, seguradora por seguradora, ANTES de decidir qualquer taxa ou multiplicador (um PDF pode ter várias páginas — transcreva página por página se precisar):
   a) para cada linha/seguradora, diga o status geral (aprovado, pré-aprovado, recusado, indisponível, limite excedido);
   b) FORMATO COMUM destes painéis: a 1ª coluna traz as seguradoras; depois, plano Básico e plano Completo são DUAS grades de multiplicador INDEPENDENTES lado a lado, cada uma tipicamente com 3 colunas de LMI (18x, 24x e 30x) — mas sempre confira os cabeçalhos reais do caso, porque podem variar. Uma seguradora pode ser aprovada em até 3 opções de LMI no Básico e mais até 3 opções de LMI no Completo, cada combinação seguradora+plano+multiplicador é independente das demais. Trate cada grade (Básico e Completo) separadamente, mesmo sendo da mesma seguradora.
   c) REGRA BÁSICA (a mais importante): pense nisso como batalha naval — cada seguradora é uma LINHA (coordenada de linha) e cada LMI (18x/24x/30x) é uma COLUNA (coordenada de coluna); o preço é o "navio" que pode estar ou não numa coordenada específica. O dado válido de cada multiplicador é exatamente o texto que está no cruzamento daquela linha (seguradora) com aquela coluna (LMI) — nada além disso. Percorra coordenada por coordenada, célula por célula, cabeçalho de coluna por cabeçalho de coluna, da esquerda para a direita. Se o cruzamento linha×coluna estiver vazio/cinza/sem "navio", IGNORE esse multiplicador por completo para aquela seguradora/plano — não crie uma linha pra ele na tabela final, e nunca empreste o "navio" de uma coordenada vizinha (linha de cima, linha de baixo, ou outra coluna) pra uma coordenada vazia.
   d) BUG CONHECIDO NESSES PAINÉIS: alguns sistemas de origem imprimem um texto de multiplicador (ex: "30X") dentro do valor de cada célula preenchida que pode estar ERRADO/travado (sempre o mesmo texto, independente da coluna real). Ainda assim, siga a regra básica do item (c): o multiplicador correto é o da COLUNA onde a célula preenchida está, contado pelo cabeçalho (1ª coluna da esquerda = primeiro multiplicador do cabeçalho, e assim por diante) — não o texto que aparece dentro da célula. Diga explicitamente na leitura_bruta qual coluna você contou para cada célula preenchida.
   e) se não estiver 100% seguro de em qual coluna uma célula preenchida está (colunas estreitas, imagem comprimida ou baixa qualidade), diga isso explicitamente aqui em vez de adivinhar.
...
1. TABELA COMPARATIVA ("opcoes") — uma linha por cotação EFETIVAMENTE APROVADA no panorama (mesma seguradora com LMIs ou planos diferentes conta como linhas separadas — Básico e Completo de uma seguradora são linhas independentes, cada um com seu próprio LMI aprovado, conforme a leitura_bruta), ORDENADA da MELHOR condição de preço (menor taxa) para a PIOR (maior taxa). Campos por linha:
