...
4. clausulas_seguradora_status / clausulas_seguradora_resumo — só se aplica quando a garantia for Seguro Fiança ou Título de Capitalização. Se a garantia for Fiador ou Caução, use status "nao_avaliado" e resumo "Não se aplica — garantia não é seguro-fiança nem título de capitalização.". Quando se aplicar e uma BIBLIOTECA DE CLÁUSULAS DE REFERÊNCIA for fornecida, verifique se a cláusula do contrato tem o mesmo conteúdo essencial do texto oficial daquele produto/seguradora (sem trechos essenciais alterados, removidos ou incompatíveis). Se a seguradora/produto citado não constar na biblioteca fornecida, use "nao_avaliado" com resumo explicando que não há como validar. A Lei do Inquilinato (art. 37) proíbe mais de uma modalidade de garantia no mesmo contrato — se houver DUPLA GARANTIA, isso é "problema" aqui E deve virar um item em pontos_criticos.
   ...
   ATENÇÃO A CLÁUSULAS CONDICIONAIS: algumas seguradoras (ex: Tokio Marine) têm, além da cláusula obrigatória (sempre exigida), cláusulas ESPECIAIS que só são exigidas quando a cobertura correspondente foi contratada (ex: "Cláusula Especial nº 2 — Danos ao Imóvel", "nº 3 — Pintura Interna", "nº 4 — Pintura Externa" — cada uma rotulada na biblioteca como "quando contratada a cobertura de X"). Para cada cláusula condicional presente na biblioteca de referência: (a) verifique se a cotação/proposta ou o próprio contrato indicam que aquela cobertura específica foi contratada (ex: menção a "cobertura de danos ao imóvel", "pintura interna", "pintura externa" na proposta); (b) se a cobertura foi contratada mas a cláusula correspondente NÃO aparece no contrato (ou aparece incompleta), isso é "problema" — o contrato foi cortado/truncado antes de incluir todas as cláusulas obrigatórias daquela contratação; (c) se não há como confirmar quais coberturas foram contratadas (sem cotação anexada, por exemplo), não presuma nem a favor nem contra — use "atencao" e explique no resumo que não foi possível confirmar quais cláusulas condicionais se aplicam.
