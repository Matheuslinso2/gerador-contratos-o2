...
3. conferencia_cotacao_status / conferencia_cotacao_resumo — o resumo deste pilar SEMPRE precisa trazer, de forma explícita, o nome do locador, o nome do locatário e o endereço do imóvel identificados no contrato — em todo relatório, tenha cotação anexada ou não. Isso é pra quem está lendo poder comparar na hora com a cotação que tem em mãos, sem precisar procurar essa informação em outro lugar do relatório.

   - Se uma COTAÇÃO/PROPOSTA DE SEGURO for fornecida abaixo: compare nome do segurado/locatário, valor do aluguel, prazo e endereço entre o contrato e a cotação, e diga o resultado nomeando os dois lados sempre que houver qualquer diferença — nunca diga só "diverge" ou "não confere" sem apontar os valores. Ex: "Locatário no contrato: João Silva; na cotação: João da Silva Santos — confira se é a mesma pessoa." ou "Endereço do contrato: Rua X, 100; da cotação: Rua X, 100, Bloco 2 — confirme se é a mesma unidade.". Se um nome existir só em um dos dois documentos (falta ou sobra), aponte isso explicitamente também, citando o nome. Status "ok" se locador, locatário, valor, prazo e endereço batem completamente entre os dois documentos; "atencao" ou "problema" (conforme a gravidade) se houver qualquer divergência de nome, valor, prazo ou endereço.
   - Se NENHUMA cotação for fornecida: use status "nao_avaliado", e o resumo deve seguir o formato "Sem cotação anexada para conferência automática — confira manualmente: Locador: [nome]. Locatário: [nome]. Endereço: [endereço].".
