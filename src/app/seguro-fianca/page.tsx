...
function tendenciaSimples(
  atual: number,
  anterior: number,
): { pct: number; direcao: "up" | "down" | "flat" } {
...
function fmtTendencia(atual: number, anterior: number): string {
  ...
  const pct =
    t.direcao === "flat"
      ? ""
      : ` ${t.pct.toLocaleString("pt-BR", { maximumFractionDigits: 0 })}%`;
  ...
  const limite = new Promise<never>((_, reject) => {
    timer = setTimeout(
      () =>
        reject(
          new Error(
            `Tempo esgotado após ${Math.round(ms / 1000)}s buscando no Bitrix`,
          ),
        ),
      ms,
    );
...
function normalizarSnapshot(
  ...
  const contratosRecebidosPorDia =
    payload.contratosRecebidosPorDia as unknown as
      { mesAtual?: unknown; herdado?: unknown } | undefined;
  return {
    ...payload,
    kpis: {
      ...payload.kpis,
      ...
      imobiliariasAtivas:
        payload.kpis.imobiliariasAtivas ?? payload.topImobiliarias.length,
    },
    analisesDiariasPorResponsavel: normalizarQuadroDiario(
      payload.analisesDiariasPorResponsavel,
      competencia,
    ),
    contratosRecebidosPorDia: {
      mesAtual: normalizarQuadroDiario(
        contratosRecebidosPorDia?.mesAtual,
        competencia,
      ),
      herdado: normalizarQuadroDiario(
        contratosRecebidosPorDia?.herdado,
        competencia,
      ),
    },
    efetivacoesPorDia: normalizarQuadroDiario(
      payload.efetivacoesPorDia,
      competencia,
    ),
...
function construirSegmentosFunil(
  ...
  for (const etapa of ETAPAS.filter(
    (e) => e.statusId.startsWith(`DT1042_${categoria}:`) && e.semantica === "P",
  )) {
  ...
  if (categoria === CATEGORIA_ANALISE) {
    if (gerencial.kpis.aprovados.total > 0) {
      segmentos.push({
        label: "Aprovado p/ Negociação e Contrato",
        value: gerencial.kpis.aprovados.total,
        classe: styles.segInfo,
      });
    }
    if (gerencial.kpis.recusados.total > 0) {
      segmentos.push({
        label: "Recusado",
        value: gerencial.kpis.recusados.total,
        classe: styles.segNeg,
      });
    }
  } else {
    if (gerencial.kpis.convertidos.total > 0) {
      segmentos.push({
        label: "Convertido",
        value: gerencial.kpis.convertidos.total,
        classe: styles.segInfo,
      });
    }
    if (gerencial.kpis.perdidos.total > 0) {
      segmentos.push({
        label: "Negativado",
        value: gerencial.kpis.perdidos.total,
        classe: styles.segNeg,
      });
    }
  }
  ...
  return (
    <div className={styles.barrow} title={tip}>
      ...
      <div className={styles.track}>
        <div
          className={`${styles.fill} ${warning ? styles.fillWarning : ""}`}
          style={{ width: `${pct}%` }}
        />
      </div>
...
function StackBar({
  ...
  segments: { label: string; value: number; classe: string }[];
  total: number;
}) {
  return (
...
function ProdutividadeTabela({
  dados,
}: {
  dados: AnaliseGerencial["porResponsavelFunil1"];
}) {
  const linhas = Object.entries(dados).sort(
    (a, b) =>
      ...
      b[1].negativos +
      b[1].positivos -
      (a[1].novos + a[1].andamento + a[1].negativos + a[1].positivos),
  );
  return (
...
function ContratosRecebidosTabela({
  mesAtual,
  herdado,
}: {
  mesAtual: QuadroDiario;
  herdado: QuadroDiario;
}) {
...
function QuadroDiarioTabela({
  ...
  const semNenhumRegistro = mostrarResponsaveis
    ? quadro.responsaveis.length === 0
    : quadro.dias.every((d) => d.total === 0);
  return (
    <>
      <div className={styles.tableWrap}>
        <table className={styles.data}>
...
export default async function SeguroFiancaPage({
  ...
  if (ehCompetenciaAtual) {
    try {
      gerencial = await comLimiteDeTempo(
        buscarAnaliseGerencialAoVivo(competencia),
      ...
      const { error: erroUpsert } = await supabase
        ...
        .upsert(
          { competencia, atualizado_em: atualizadoEm, payload: gerencial },
          { onConflict: "competencia" },
        );
      if (erroUpsert) {
        console.error(
          "Falha ao salvar snapshot do Seguro Fiança no Supabase:",
          erroUpsert,
        );
      }
    } catch (e) {
      erro =
        e instanceof Error ? e.message : "Falha ao buscar dados do Bitrix.";
      ...
      const { data } = await supabase
        ...
        .maybeSingle();
      if (data) {
        gerencial = normalizarSnapshot(
          data.payload as AnaliseGerencial & { totalMovimentacoes: number },
          competencia,
        );
    ...
    if (data) {
      ...
      gerencial = {
        ...montarAnaliseGerencial([], [], competencia),
  ...
  const { data: snapshotAnterior } = await supabase
    ...
    .maybeSingle();
  const topImobiliariasMesAnterior =
    (snapshotAnterior?.payload as AnaliseGerencial | undefined)
      ?.topImobiliarias ?? [];
        ...
        <div id="painel-fianca-completo" className={styles.container}>
          <div className={styles.masthead}>
            <PageHeader
              ...
              subtitulo={`Painel de Produção — ${competencia} (SPA Bitrix24)`}
            />
            <div className={styles.meta}>
              ...
              >
                ...
                {gerencial && (
                  <BotaoExportarPainelPdf
                    painelId="painel-fianca-completo"
                    nomeArquivo={`seguro-fianca-painel-${competencia}`}
                    corFundo="#f7f8fa"
                  />
                )}
              </div>
              ...
              {semRegistroNoPeriodo && (
                <>Nenhum card registrado nesta competência</>
              )}
            </div>
          </div>
          ...
          {erro && (
            <div
              className={styles.stampPanel + " " + styles.stampPanelWarning}
              style={{ marginBottom: 24 }}
            >
              <div
                className={styles.stampBadge + " " + styles.stampBadgeWarning}
              >
                ERRO
              </div>
          ...
          {gerencial && (
            <AbasProvider abas={ABAS}>
              <AbaSlot aba="visao-geral">
                <div
                  id="quadro-fianca-total-analises"
                    ...
                    dadosExcel={[
                      {
                        ...
                        valor: gerencial.kpis.total,
                      },
                      {
                        indicador: "Imobiliárias",
                        valor: gerencial.kpis.imobiliarias,
                      },
                      {
                        indicador: "Em Andamento",
                        valor: gerencial.kpis.emAndamento.mesAtual,
                      },
                      {
                        indicador: "Recusados",
                        valor: gerencial.kpis.recusados.mesAtual,
                      },
                      {
                        indicador: "Aprovados",
                        valor: gerencial.kpis.aprovados.mesAtual,
                      },
                      {
                        indicador: "Negativados",
                        valor: gerencial.kpis.perdidos.mesAtual,
                      },
                      {
                        indicador: "Convertidos",
                        valor: gerencial.kpis.convertidos.mesAtual,
                      },
                      {
                        indicador: "Cards com Alerta",
                        valor: gerencial.kpis.comAlerta,
                      },
                    ]}
                ...
                <div id="quadro-fianca-novidades" className={styles.kpis}>
                  <Kpi
                    ...
                    value={String(gerencial.kpis.total)}
                    sub="criados nesta competência"
                  />
                  <Kpi
                    label="Imobiliárias"
                  ...
                  <Kpi
                    ...
                    tone={
                      gerencial.kpis.comAlerta /
                        Math.max(gerencial.kpis.totalRelevantes, 1) >
                      0.2
                        ? "warning"
                        : "positive"
                    }
                ...
                >
                  ...
                  <ExportarQuadro
                    ...
                    dadosExcel={[
                      {
                        indicador: "Imobiliárias",
                        valor: gerencial.kpis.imobiliariasHerdado,
                      },
                      {
                        indicador: "Em Andamento",
                        valor: gerencial.kpis.emAndamento.herdado,
                      },
                      {
                        indicador: "Recusados",
                        valor: gerencial.kpis.recusados.herdado,
                      },
                      {
                        indicador: "Aprovados",
                        valor: gerencial.kpis.aprovados.herdado,
                      },
                      {
                        indicador: "Negativados",
                        valor: gerencial.kpis.perdidos.herdado,
                      },
                      {
                        indicador: "Convertidos",
                        valor: gerencial.kpis.convertidos.herdado,
                      },
                    ]}
              ...
              {(() => {
                ...
                const segmentosFunil1 = construirSegmentosFunil(
                  gerencial,
                  CATEGORIA_ANALISE,
                );
                const segmentosFunil2 = construirSegmentosFunil(
                  gerencial,
                  CATEGORIA_NEGOCIACAO,
                );
                const totalFunil1 = segmentosFunil1.reduce(
                  (a, s) => a + s.value,
                  0,
                );
                const totalFunil2 = segmentosFunil2.reduce(
                  (a, s) => a + s.value,
                  0,
                );
                return (
                  <>
                        ...
                        <div className={styles.panel}>
                          <div className={styles.stackGroup}>
                            <div className={styles.glabel}>
                              <span>
                                Análise e Cotação — em andamento (todos) +
                                recusado/aprovado deste mês
                              </span>
                    ...
                    <AbaSlot aba="fechamento">
                      <section
                        id="quadro-fianca-distribuicao-fechamento"
                        className={styles.section}
                      >
                        <div className={styles.sectionHead}>
                          <h2>
                            Distribuição por etapa — Negociação e Contrato
                          </h2>
                          <div className={styles.note}>
                            em andamento (todos) + convertido/negativado deste
                            mês
                          </div>
                          <ExportarQuadro
                            ...
                            dadosExcel={segmentosFunil2.map((s) => ({
                              etapa: s.label,
                              cards: s.value,
                            }))}
                        ...
                        <div className={styles.panel}>
                          <div className={styles.stackGroup}>
                            <div className={styles.glabel}>
                              <span>
                                Negociação e Contrato — em andamento (todos) +
                                convertido/negativado deste mês
                              </span>
                              <span className={styles.num}>
                                {totalFunil2} cards
                              </span>
              ...
              <AbaSlot aba="cotacoes">
                <section
                  id="quadro-fianca-analise-resumo-cotacoes"
                  className={styles.section}
                >
                  <div className={styles.sectionHead}>
                    <h2>Análise, recusa e aprovação</h2>
                    <div className={styles.note}>
                      total de análise = todo o conjunto ativo do mês (novidades
                      + herdados ativos)
                    </div>
                    <ExportarQuadro
                      ...
                      dadosExcel={[
                        {
                          indicador: "Total de Análise",
                          valor: gerencial.kpis.totalRelevantes,
                        },
                        {
                          indicador: "Aprovados",
                          valor: gerencial.kpis.aprovados.total,
                        },
                        {
                          indicador: "Recusados",
                          valor: gerencial.kpis.recusados.total,
                        },
                      ]}
                  ...
                  <div className={styles.kpis}>
                    <Kpi
                      label="Total de Análise"
                      value={String(gerencial.kpis.totalRelevantes)}
                      sub="novidades + herdados ativos no mês"
                    />
                    <Kpi
                      label="Aprovados"
                      value={String(gerencial.kpis.aprovados.total)}
                      sub="passaram p/ Negociação, mês do evento"
                      tone="info"
                    />
                    <Kpi
                      label="Recusados"
                      value={String(gerencial.kpis.recusados.total)}
                      sub="seguradora negou, mês do evento"
                      tone="negative"
                    />
                  </div>
                </section>
              ...
              <AbaSlot aba="seguradora">
                <section
                  id="quadro-fianca-seguradora-plano"
                  className={styles.section}
                >
                  <div className={styles.sectionHead}>
                    <h2>Análise por seguradora e plano</h2>
                    <div className={styles.note}>
                      status de cotação — Porto e Pottencial têm mais de um
                      plano cotado por card
                    </div>
                    <ExportarQuadro
                      quadroId="quadro-fianca-seguradora-plano"
                      corFundo="#f7f8fa"
                      nomeArquivo={`seguro-fianca-seguradora-plano-${competencia}`}
                      dadosExcel={Object.entries(
                        gerencial.statusPorSeguradora,
                      ).flatMap(([nome, contagens]) =>
                        Object.entries(contagens).map(
                          ([status, quantidade]) => ({
                            seguradora: nome,
                            status,
                            quantidade,
                  ...
                  <div className={styles.panel}>
                    {Object.entries(gerencial.statusPorSeguradora).map(
                      ([nome, contagens]) => {
                        const OK = new Set([
                          "Aprov",
                          "Aprov.",
                        ...
                        return (
                          <div key={nome} className={styles.insRow}>
                            ...
                            <div className={styles.insTrack}>
                              {ok > 0 && (
                                <div
                                  className={`${styles.insSeg} ${styles.insOk}`}
                                  style={{
                                    width: `${(ok / gerencial.kpis.total) * 100}%`,
                                  }}
                                  title={`Aprovado/Pré-aprovado: ${ok}`}
                                />
                              )}
                              {no > 0 && (
                                <div
                                  className={`${styles.insSeg} ${styles.insNo}`}
                                  style={{
                                    width: `${(no / gerencial.kpis.total) * 100}%`,
                                  }}
                                  title={`Recusado: ${no}`}
                                />
                              )}
                              {pend > 0 && (
                                <div
                                  className={`${styles.insSeg} ${styles.insPend}`}
                                  style={{
                                    width: `${(pend / gerencial.kpis.total) * 100}%`,
                                  }}
                                  title={`Pendente/Em análise: ${pend}`}
                                />
                              )}
                              {skip > 0 && (
                                <div
                                  className={`${styles.insSeg} ${styles.insSkip}`}
                                  style={{
                                    width: `${(skip / gerencial.kpis.total) * 100}%`,
                                  }}
                                  title={`Não analisar: ${skip}`}
                                />
                              )}
                            </div>
                    ...
                    <div className={styles.legendRow} style={{ marginTop: 14 }}>
                      ...
                      <div className={styles.legendItem}>
                        ...
                        />{" "}
                        Pendente/Em análise/Limite insuficiente
                      </div>
                      <div className={styles.legendItem}>
                        <span
                          className={styles.swatch}
                ...
                <section
                  id="quadro-fianca-top3-pottencial"
                  className={styles.section}
                >
                  <div className={styles.sectionHead}>
                    ...
                    <ExportarQuadro
                      ...
                      dadosExcel={gerencial.top3AprovacaoPottencial.map(
                        (im) => ({
                          imobiliaria: im.nome,
                          aprovados_pottencial: im.aprovados,
                        }),
                      )}
                    ...
                    <div className={styles.barlist}>
                      {gerencial.top3AprovacaoPottencial.map((im) => (
                        <BarraProporcional
                          ...
                          value={im.aprovados}
                          max={
                            gerencial.top3AprovacaoPottencial[0]?.aprovados ?? 1
                          }
                        />
                      ))}
                      {gerencial.top3AprovacaoPottencial.length === 0 && (
                        <div
                          style={{ color: "var(--ink-faint)", fontSize: 12.5 }}
                        >
                          Nenhuma aprovação na Pottencial neste período.
                        </div>
                      )}
                    </div>
                  </div>
              ...
              <AbaSlot aba="equipe">
                <section
                  id="quadro-fianca-produtividade"
                  className={styles.section}
                >
                  <div className={styles.sectionHead}>
                    <h2>Produtividade por responsável, por funil</h2>
                    <div className={styles.note}>
                      Análise e Cotação: Responsável(is) pela Cotação, ou pelo
                      Cadastro se a cotação ainda não foi atribuída. Negociação
                      e Contrato: Responsável(is) pela Negociação. Nunca o dono
                      atual do card.
                    </div>
                    <ExportarQuadro
                      ...
                      dadosExcel={[
                        ...Object.entries(gerencial.porResponsavelFunil1).map(
                          ([nome, d]) => ({
                        ...
                        ...Object.entries(gerencial.porResponsavelFunil2).map(
                          ([nome, d]) => ({
                  ...
                  <div className={styles.grid2}>
                    <div className={styles.panel}>
                      ...
                      <div className={styles.panelSub}>
                        Positivos = card que essa pessoa encaminhou pro funil 2
                        (crédito de quem concluiu, não de quem só criou)
                      </div>
                      <ProdutividadeTabela
                        dados={gerencial.porResponsavelFunil1}
                      />
                    </div>
                    <div className={styles.panel}>
                      <h3>Negociação e Contrato</h3>
                      <div className={styles.panelSub}>
                        {gerencial.kpis.aprovados.total} cards encaminhados este
                        mês
                      </div>
                      <ProdutividadeTabela
                        dados={gerencial.porResponsavelFunil2}
                      />
                    </div>
                  </div>
                </section>
              ...
              <AbaSlot aba="seguradora">
                <section
                  id="quadro-fianca-taxa-parcela"
                  className={styles.section}
                >
                  <div className={styles.sectionHead}>
                    <h2>Taxa média da parcela sobre o pacote de locação</h2>
                    <div className={styles.note}>
                      parcela do seguro ÷ pacote de locação, por seguradora — só
                      nos cards cotados
                    </div>
                    <ExportarQuadro
                      ...
                      dadosExcel={SEGURADORAS.map((seg) => {
                        ...
                        return {
                          seguradora: seg.nome,
                          cotados: t.n,
                          taxa_sobre_pacote_locacao: t.pctLocacao,
                          taxa_sobre_aluguel: t.pctAluguel,
                        };
                      })}
                ...
                <section
                  id="quadro-fianca-cotado-convertido"
                  className={styles.section}
                >
                  <div className={styles.sectionHead}>
                    <h2>Cotado vs. Convertido, por seguradora</h2>
                    <div className={styles.note}>
                      todo prêmio cotado até agora contra o que já foi
                      efetivamente convertido
                    </div>
                    <ExportarQuadro
                      ...
                      dadosExcel={[
                        ...
                        ...Object.entries(
                          gerencial.convertidoPorSeguradora,
                        ).map(([nome, c]) => ({
                          tipo: "Convertido",
                          seguradora: nome,
                          cards: c.n,
                  ...
                  <div className={styles.panel}>
                          ...
                          >
                            ...
                            <td className={`${styles.numCol} ${styles.num}`}>
                              {fmtBRL(
                                Object.values(
                                  gerencial.cotadoPorSeguradora,
                                ).reduce((a, c) => a + c.comissao, 0),
                              )}
                            </td>
                          </tr>
                          <tr>
                            <td style={{ color: "var(--negative)" }}>
                              Total convertido (fechado)
                            </td>
                            <td className={`${styles.numCol} ${styles.num}`}>
                              {gerencial.kpis.convertidos.total}
                            </td>
                            <td className={`${styles.numCol} ${styles.num}`}>
                              {fmtBRL(
                                Object.values(
                    ...
                    <div className={styles.panelSub} style={{ marginTop: 12 }}>
                      &quot;Comissão cotada&quot; é estimada (valor × % de
                      comissão de cada seguradora) — potencial, ainda não é
                      dinheiro confirmado. O prêmio/comissão convertido usa o
                      valor líquido e a comissão final registrados no fechamento
                      do contrato.
                    </div>
                    {Object.keys(gerencial.convertidoPorSeguradora).length >
                      0 && (
                      <div style={{ marginTop: 16 }}>
                        <h3>Convertidos por seguradora</h3>
                        <div className={styles.panelSub}>
                          seguradora escolhida no fechamento — pode não bater
                          com a lista de cotação acima se o campo não foi
                          preenchido
                        </div>
                        <div className={styles.tableWrap}>
                          <table className={styles.data}>
              ...
              <AbaSlot aba="cotacoes">
                <section id="quadro-fianca-valores" className={styles.section}>
                  <div className={styles.sectionHead}>
                    ...
                    <div className={styles.note}>
                      não é receita — ver nota no rodapé
                    </div>
                    <ExportarQuadro
                  ...
                  <div className={styles.panel}>
                    ...
                    <div className={styles.panelSub}>
                      quanto maior o pacote de locação, maior a parcela média do
                      seguro cotado
                    </div>
                    <div className={styles.tableWrap}>
                      <table className={styles.data}>
                    ...
                    <div className={styles.panelSub} style={{ marginTop: 12 }}>
                      Totais do mês: aluguel trabalhado{" "}
                      {fmtBRL(gerencial.valoresTrabalhados.aluguel)}, pacote de
                      locação trabalhado{" "}
                      {fmtBRL(gerencial.valoresTrabalhados.pacoteLocacao)}.
                    </div>
                  </div>
                </section>
                      ...
                      dadosExcel={[
                        {
                          ...
                          media_min:
                            gerencial.tempoPorFunil.analiseECotacao.media,
                          mediana_min:
                            gerencial.tempoPorFunil.analiseECotacao.mediana,
                          minimo_min:
                            gerencial.tempoPorFunil.analiseECotacao.min,
                          maximo_min:
                            gerencial.tempoPorFunil.analiseECotacao.max,
                        },
                      ]}
                      ...
                      <table className={styles.data}>
                        ...
                        <tbody>
                          <tr>
                            ...
                            <td className={`${styles.numCol} ${styles.num}`}>
                              {gerencial.tempoPorFunil.analiseECotacao.n}
                            </td>
                            <td className={`${styles.numCol} ${styles.num}`}>
                              {fmtDuracao(
                                gerencial.tempoPorFunil.analiseECotacao.media,
              ...
              <AbaSlot aba="fechamento">
                <section
                  id="quadro-fianca-tempo-ciclo-fechamento"
                  className={styles.section}
                >
                  <div className={styles.sectionHead}>
                    <h2>Tempo de ciclo — Negociação e Contrato</h2>
                    <div className={styles.note}>
                      do início do funil 2 até converter/perder
                    </div>
                    <ExportarQuadro
                      ...
                      dadosExcel={[
                        {
                          ...
                          media_min:
                            gerencial.tempoPorFunil.negociacaoEContrato.media,
                          mediana_min:
                            gerencial.tempoPorFunil.negociacaoEContrato.mediana,
                          minimo_min:
                            gerencial.tempoPorFunil.negociacaoEContrato.min,
                          maximo_min:
                            gerencial.tempoPorFunil.negociacaoEContrato.max,
                        },
                      ]}
                      nomeAbaExcel="Tempo de ciclo"
                    />
                  ...
                  <div className={styles.panel}>
                    <div className={styles.tableWrap}>
                      <table className={styles.data}>
                        ...
                        <tbody>
                          <tr>
                            <td>
                              Negociação e Contrato (até converter/perder)
                            </td>
                            <td className={`${styles.numCol} ${styles.num}`}>
                              {gerencial.tempoPorFunil.negociacaoEContrato.n}
                            </td>
                            <td className={`${styles.numCol} ${styles.num}`}>
                              {fmtDuracao(
                                gerencial.tempoPorFunil.negociacaoEContrato
                                  .media,
                              )}
                            </td>
                            <td className={`${styles.numCol} ${styles.num}`}>
                    ...
                    <div className={styles.panelSub} style={{ marginTop: 12 }}>
                      &quot;Cards&quot; conta quem já tem esse tempo definido —
                      só existe pra quem foi aprovado em Análise e Cotação.
                    </div>
                  </div>
                </section>
              </AbaSlot>
              ...
              <AbaSlot aba="equipe">
                <section
                  id="quadro-fianca-tempo-cotacao"
                  className={styles.section}
                >
                  <div className={styles.sectionHead}>
                    <h2>Tempo de cotação por responsável</h2>
                    <div className={styles.note}>
                      HORA INICIO → HORA FIM da fase de cotação, por
                      Responsável(is) pela Cotação — separado por resultado
                      porque recusar é mais rápido que cotar de verdade
                    </div>
                    <ExportarQuadro
                      ...
                      dadosExcel={Object.entries(
                        gerencial.tempoCotacaoPorResponsavel,
                      ).map(([nome, d]) => ({
                        responsavel: nome,
                        recusados: d.recusado.n,
                        tempo_medio_recusados_min:
                          d.recusado.n > 0 ? d.recusado.media : "",
                        aprovados_liberados: d.aprovado.n,
                        tempo_medio_aprovados_min:
                          d.aprovado.n > 0 ? d.aprovado.media : "",
                      }))}
                  ...
                  <div className={styles.panel}>
                          ...
                          {Object.entries(gerencial.tempoCotacaoPorResponsavel)
                            ...
                            .map(([nome, d]) => (
                              <tr key={nome}>
                                ...
                                >
                                  {d.aprovado.n > 0
                                    ? fmtDuracao(d.aprovado.media)
                                    : "—"}
                                </td>
                              </tr>
                            ))}
                          {Object.keys(gerencial.tempoCotacaoPorResponsavel)
                            .length === 0 && (
                            <tr>
                              <td
                                colSpan={5}
                              ...
                              >
                                Nenhum card com HORA INICIO e HORA FIM de
                                cotação preenchidos neste período.
                              </td>
                            </tr>
                          )}
                        </tbody>
                    ...
                    <div className={styles.panelSub} style={{ marginTop: 12 }}>
                      {gerencial.qualidade.cotacaoTempoInconsistente > 0 ? (
                        <>
                          {gerencial.qualidade.cotacaoTempoInconsistente}{" "}
                          card(s) com HORA INICIO e HORA FIM aparentemente
                          preenchidos trocados — duração corrigida
                          automaticamente (valor absoluto), já incluída nas
                          médias.
                        </>
                      ) : (
                        <>
                          Campo recente (desde 05/08/2026) — a amostra cresce a
                          cada dia.
                        </>
                      )}
                    </div>
                ...
                {(() => {
                  ...
                  const nomes = new Set([
                    ...Object.keys(gerencial.tempoCotacaoPorResponsavel),
                    ...Object.keys(gerencial.porResponsavelFunil1),
                  ]);
                  const linhas = [...nomes]
                    .map((nome) => {
                      const t = gerencial.tempoCotacaoPorResponsavel[nome];
                      const nComTempo =
                        (t?.recusado.n ?? 0) + (t?.aprovado.n ?? 0);
                      const mediaRetorno =
                        nComTempo > 0
                          ? Math.round(
                              (((t?.recusado.media ?? 0) *
                      ...
                      return {
                        ...
                        nComTempo,
                        totalAnalises:
                          gerencial.porResponsavelFunil1[nome]?.novos ?? 0,
                      };
                    })
                    .filter((l) => l.totalAnalises > 0 || l.nComTempo > 0)
                    .sort((a, b) => b.totalAnalises - a.totalAnalises);
                  const somaPonderada = linhas.reduce(
                    (a, l) => a + (l.mediaRetorno ?? 0) * l.nComTempo,
                    0,
                  );
                  const somaN = linhas.reduce((a, l) => a + l.nComTempo, 0);
                  const mediaGeralTime =
                    somaN > 0
                      ? Math.round((somaPonderada / somaN) * 10) / 10
                      : null;
                  return (
                    <section
                      id="quadro-fianca-retorno-analista"
                      className={styles.section}
                    >
                      <div className={styles.sectionHead}>
                        <h2>Tempo de retorno e produção por analista</h2>
                        <div className={styles.note}>
                          tempo de retorno = média entre recusados e
                          aprovados/liberados (HORA INICIO → HORA FIM); total de
                          análises = cotações novas criadas neste mês atribuídas
                          à pessoa
                        </div>
                        <ExportarQuadro
                              ...
                              {linhas.length === 0 && (
                                <tr>
                                  <td
                                    ...
                                    style={{ color: "var(--ink-faint)" }}
                                  >
                                    Nenhum analista com dado neste período.
                                  </td>
                                </tr>
                              )}
                            </tbody>
                ...
                <section id="quadro-fianca-diarios" className={styles.section}>
                  ...
                  <div className={styles.grid3}>
                    ...
                    <div className={styles.panel}>
                      ...
                      <div className={styles.panelSub}>
                        cards que entraram na etapa &quot;Contrato
                        Recebido&quot; — Mês Atual conta os cards criados nesta
                        competência, Mês Anterior conta os herdados de qualquer
                        competência anterior
                      </div>
                      <ContratosRecebidosTabela
                        mesAtual={gerencial.contratosRecebidosPorDia.mesAtual}
                        herdado={gerencial.contratosRecebidosPorDia.herdado}
                      />
                    </div>
                    <div className={styles.panel}>
                      <h3>Efetivações por dia</h3>
                      <div className={styles.panelSub}>
                        dia da Data de Efetivação, por Responsável pela
                        Efetivação — sem o campo preenchido vira alerta em
                        &quot;Qualidade dos dados&quot;, não entra na quebra por
                        pessoa
                      </div>
                      <QuadroDiarioTabela
                        quadro={gerencial.efetivacoesPorDia}
                      />
                    </div>
                  </div>
                </section>
              ...
              <AbaSlot aba="fechamento">
                <section
                  id="quadro-fianca-contratos-tardios"
                  className={styles.section}
                >
                  <div className={styles.sectionHead}>
                    <h2>Contratos e contratação tardios</h2>
                    <div className={styles.note}>
                      contrato tardio = criado em mês anterior, entrou em
                      &quot;Contrato Recebido&quot; este mês. Contratação tardia
                      = recebeu o contrato num mês anterior, mas só converteu
                      (fechou) este mês
                    </div>
                    <ExportarQuadro
                      ...
                      dadosExcel={[
                        ...
                        {
                          indicador: "% s/ Total de Fechamentos",
                          valor:
                            gerencial.contratosTardios
                              .pctContratacaoTardiaSobreFechamentos,
                        },
                        {
                          indicador: "Prêmio Líquido (Contratação Tardia)",
                          valor:
                            gerencial.contratosTardios
                              .premioLiquidoContratacaoTardia,
                        },
                        {
                          indicador: "Negociação (Tardia)",
                          valor: gerencial.contratosTardios.negociacaoTardia,
                        },
                        {
                          indicador: "Ag. Contrato (Tardia)",
                          valor: gerencial.contratosTardios.agContratoTardia,
                        },
                      ]}
                  ...
                  <div className={styles.kpis}>
                    ...
                    <Kpi
                      ...
                      value={String(
                        gerencial.contratosTardios.contratacaoTardia,
                      )}
                    ...
                    />
                    <Kpi
                      label="Negociação (Tardia)"
                      value={String(
                        gerencial.contratosTardios.negociacaoTardia,
                      )}
                      sub="herdados parados em Em Negociação"
                    />
                    <Kpi
                      label="Ag. Contrato (Tardia)"
                      value={String(
                        gerencial.contratosTardios.agContratoTardia,
                      )}
                      sub="herdados parados em Aguardando Contrato"
                    />
                  </div>
                </section>
              </AbaSlot>
              ...
              <AbaSlot aba="cotacoes">
                <section
                  id="quadro-fianca-motivos-recusa"
                  className={styles.section}
                >
                  <div className={styles.sectionHead}>
                    <h2>Motivos de recusa</h2>
                    <div className={styles.note}>
                      {gerencial.motivosRecusaFunil1.total} recusados em Análise
                      e Cotação
                    </div>
                  </div>
                  <div className={styles.panel}>
                    ...
                    <div className={styles.panelSub}>
                      decisão de compliance da seguradora, sem motivo interno
                      esperado
                    </div>
                    <div className={styles.barlist}>
                      <BarraProporcional
                        label="Total de recusas"
                        value={gerencial.motivosRecusaFunil1.total}
                        max={Math.max(gerencial.motivosRecusaFunil1.total, 1)}
                      />
                    </div>
                  </div>
                </section>
              ...
              <AbaSlot aba="fechamento">
                <section
                  id="quadro-fianca-motivos-negativacao"
                  className={styles.section}
                >
                  <div className={styles.sectionHead}>
                    <h2>Motivos de negativação</h2>
                    <div className={styles.note}>
                      {gerencial.motivosPerdaFunil2.total} negativados em
                      Negociação e Contrato
                    </div>
                    <ExportarQuadro
                      ...
                      dadosExcel={[
                        ...Object.entries(
                          gerencial.motivosPerdaFunil2.porMotivo,
                        ).map(([motivo, n]) => ({
                          motivo,
                          cards: n,
                        })),
                          ...
                          ? [
                              {
                                ...
                                cards: gerencial.motivosPerdaFunil2.semMotivo,
                              },
                            ]
                          : []),
                      ]}
                  ...
                  <div className={styles.panel}>
                    ...
                    <div className={styles.panelSub}>
                      {gerencial.motivosPerdaFunil2.total} cards negativados
                    </div>
                    <div className={styles.barlist}>
                      {Object.entries(
                        gerencial.motivosPerdaFunil2.porMotivo,
                        ...
                        <BarraProporcional
                          ...
                          label={motivo}
                          value={n}
                          max={gerencial.motivosPerdaFunil2.total}
                        />
                      ))}
                      {gerencial.motivosPerdaFunil2.semMotivo > 0 && (
                        <BarraProporcional
                          label="Sem motivo registrado"
                          value={gerencial.motivosPerdaFunil2.semMotivo}
                          max={gerencial.motivosPerdaFunil2.total}
                          warning
                        />
                      )}
                      {gerencial.motivosPerdaFunil2.total === 0 && (
                        <div
                          style={{ color: "var(--ink-faint)", fontSize: 12.5 }}
                        >
                          Nenhuma negativação registrada neste período.
                        </div>
                      )}
                    </div>
                  </div>
              ...
              <AbaSlot aba="imobiliaria">
                ...
                {(() => {
                  ...
                  const linhas = gerencial.topImobiliarias
                    .filter((im) => im.clienteNovo && im.total > 0)
                    .sort((a, b) => b.total - a.total);
                  return (
                    <section
                      id="quadro-fianca-clientes-novos"
                      className={styles.section}
                    >
                      <div className={styles.sectionHead}>
                        <h2>Clientes novos</h2>
                        <div className={styles.note}>
                          imobiliárias cuja 1ª cotação de toda a história caiu
                          neste mês
                        </div>
                        <ExportarQuadro
                              ...
                              {linhas.length === 0 && (
                                <tr>
                                  <td
                                    ...
                                    style={{ color: "var(--ink-faint)" }}
                                  >
                                    Nenhum cliente novo neste período.
                                  </td>
                                </tr>
                              )}
                            </tbody>
                ...
                {(() => {
                  ...
                  const nomes = new Set([
                    ...gerencial.topImobiliarias.map((im) => im.nome),
                    ...topImobiliariasMesAnterior.map((im) => im.nome),
                  ]);
                  const comparativo = [...nomes]
                    .filter((n) => n !== NOME_NAO_ADMINISTRADA)
                    .map((nome) => {
                      const atual = gerencial.topImobiliarias.find(
                        (im) => im.nome === nome,
                      );
                      const anterior = topImobiliariasMesAnterior.find(
                        (im) => im.nome === nome,
                      );
                      return {
                        nome,
                  ...
                  const porCotacoes = comparativo.map((c) => ({
                    nome: c.nome,
                    delta: c.cotAtual - c.cotAnterior,
                    atual: c.cotAtual,
                    anterior: c.cotAnterior,
                  }));
                  const porConversoes = comparativo.map((c) => ({
                    nome: c.nome,
                    delta: c.convAtual - c.convAnterior,
                    atual: c.convAtual,
                    anterior: c.convAnterior,
                  }));
                  const top10CresCot = [...porCotacoes]
                    .filter((c) => c.delta > 0)
                    .sort((a, b) => b.delta - a.delta)
                    .slice(0, 10);
                  const top10QuedaCot = [...porCotacoes]
                    .filter((c) => c.delta < 0)
                    .sort((a, b) => a.delta - b.delta)
                    .slice(0, 10);
                  const top10CresConv = [...porConversoes]
                    .filter((c) => c.delta > 0)
                    .sort((a, b) => b.delta - a.delta)
                    .slice(0, 10);
                  const top10QuedaConv = [...porConversoes]
                    .filter((c) => c.delta < 0)
                    .sort((a, b) => a.delta - b.delta)
                    .slice(0, 10);
                  const ListaTop10 = ({
                    ...
                    <div className={styles.panel}>
                      ...
                      {itens.length === 0 ? (
                        <div
                          style={{ color: "var(--ink-faint)", fontSize: 12.5 }}
                        >
                          Nenhuma imobiliária nesse recorte.
                        </div>
                      ) : (
                        <div className={styles.tableWrap}>
                          <table
                  ...
                  return (
                    <section
                      id="quadro-fianca-top5-comparativo"
                      className={styles.section}
                    >
                      <div className={styles.sectionHead}>
                        <h2>Comparativo com o mês anterior — Top 10</h2>
                        <div className={styles.note}>
                          maior crescimento e maior queda, em cotações e em
                          conversões
                        </div>
                        <ExportarQuadro
                          ...
                          dadosExcel={[
                            ...top10CresCot.map((i) => ({
                              recorte: "Maior crescimento em cotações",
                              imobiliaria: i.nome,
                              anterior: i.anterior,
                              atual: i.atual,
                              variacao: i.delta,
                            ...
                            ...top10QuedaCot.map((i) => ({
                              recorte: "Maior queda em cotações",
                              imobiliaria: i.nome,
                              anterior: i.anterior,
                              atual: i.atual,
                              variacao: i.delta,
                            ...
                            ...top10CresConv.map((i) => ({
                              recorte: "Maior crescimento em conversões",
                              imobiliaria: i.nome,
                              anterior: i.anterior,
                              atual: i.atual,
                              variacao: i.delta,
                            })),
                            ...top10QuedaConv.map((i) => ({
                              recorte: "Maior queda em conversões",
                              imobiliaria: i.nome,
                              anterior: i.anterior,
                              atual: i.atual,
                              variacao: i.delta,
                      ...
                      <div className={styles.grid2}>
                        <ListaTop10
                          titulo="Maior crescimento em cotações"
                          itens={top10CresCot}
                        />
                        <ListaTop10
                          titulo="Maior queda em cotações"
                          itens={top10QuedaCot}
                        />
                        <ListaTop10
                          titulo="Maior crescimento em conversões"
                          itens={top10CresConv}
                        />
                        <ListaTop10
                          titulo="Maior queda em conversões"
                          itens={top10QuedaConv}
                        />
                      </div>
                    </section>
                  );
                ...
                {(() => {
                  ...
                  const nomesAtual = new Set(
                    gerencial.topImobiliarias
                      .filter(
                        (im) =>
                          im.total > 0 && im.nome !== NOME_NAO_ADMINISTRADA,
                      )
                      .map((im) => im.nome),
                  );
                  const nomesAnterior = new Set(
                    topImobiliariasMesAnterior
                      .filter(
                        (im) =>
                          im.total > 0 && im.nome !== NOME_NAO_ADMINISTRADA,
                      )
                  ...
                  const entraram = [...nomesAtual]
                    .filter((n) => !nomesAnterior.has(n))
                    .sort();
                  const sairam = [...nomesAnterior]
                    .filter((n) => !nomesAtual.has(n))
                    .sort();
                  return (
                    <section
                      id="quadro-fianca-entraram-sumiram"
                      className={styles.section}
                    >
                      <div className={styles.sectionHead}>
                        <h2>Entraram e sumiram</h2>
                        <div className={styles.note}>
                          imobiliárias que cotaram só num dos 2 meses —
                          categoria por tier ainda não existe (Fase 2)
                        </div>
                        <ExportarQuadro
                          ...
                          dadosExcel={[
                            ...entraram.map((nome) => ({
                              imobiliaria: nome,
                              situacao: "Entrou este mês",
                            })),
                            ...sairam.map((nome) => ({
                              imobiliaria: nome,
                              situacao: "Sumiu este mês",
                            })),
                          ]}
                ...
                {(() => {
                  ...
                  const totalAtual = gerencial.topImobiliarias
                  ...
                  const linhas = [...nomes]
                    .map((nome) => {
                      ...
                      return {
                        ...
                        pctAtual:
                          totalAtual > 0
                            ? ((atual?.premioEfetivado ?? 0) / totalAtual) * 100
                            : 0,
                        pctAnterior:
                          totalAnterior > 0
                            ? ((anterior?.premioEfetivado ?? 0) /
                                totalAnterior) *
                              100
                            : 0,
                      };
                  ...
                  return (
                    <section
                      id="quadro-fianca-premio-imobiliaria"
                      className={styles.section}
                    >
                      <div className={styles.sectionHead}>
                        <h2>
                          Prêmio líquido e comissão por imobiliária — mês × mês
                        </h2>
                        <div className={styles.note}>
                          participação de cada imobiliária no total efetivado do
                          mês, em valor absoluto e %
                        </div>
                        <ExportarQuadro
                              ...
                              {linhas.map((l) => (
                                <tr key={l.nome}>
                                  ...
                                  <td
                                    className={`${styles.numCol} ${styles.num}`}
                                  >
                                    {fmtBRL(l.comissaoAtual)}
                                  </td>
                                  <td
                                    className={`${styles.numCol} ${styles.num}`}
                                  >
                                    {fmtBRL(l.premioAnterior)}
                                  </td>
                                  <td
                                    className={`${styles.numCol} ${styles.num}`}
                                  >
                                    {fmtPct(l.pctAnterior)}
                                  </td>
                                </tr>
                              ))}
                              {linhas.length === 0 && (
                                <tr>
                                  <td
                                    ...
                                    style={{ color: "var(--ink-faint)" }}
                                  >
                                    Nenhum prêmio efetivado nos 2 meses.
                                  </td>
                                </tr>
                              )}
                            </tbody>
              ...
              <AbaSlot aba="visao-geral">
                <section
                  id="quadro-fianca-qualidade"
                  className={styles.section}
                >
                  <div className={styles.sectionHead}>
                    <h2>Qualidade dos dados</h2>
                    <ExportarQuadro
                      quadroId="quadro-fianca-qualidade"
                      corFundo="#f7f8fa"
                      nomeArquivo={`seguro-fianca-qualidade-${competencia}`}
                    />
                  </div>
                  {(() => {
                    ...
                    return (
                      <div
                        className={`${styles.stampPanel} ${algumAlerta ? styles.stampPanelWarning : ""}`}
                      >
                        <div
                          className={`${styles.stampBadge} ${algumAlerta ? styles.stampBadgeWarning : ""}`}
                        >
                          {algumAlerta ? (
                            <>
                              ATENÇÃO
                        ...
                        <div className={styles.stampList}>
                          <div>
                            {Math.round(
                              ((gerencial.kpis.total -
                                gerencial.qualidade.semImobiliaria) /
                                Math.max(gerencial.kpis.total, 1)) *
                                100,
                            )}
                            % dos cards (
                            {gerencial.kpis.total -
                              gerencial.qualidade.semImobiliaria}{" "}
                            de {gerencial.kpis.total}) têm imobiliária/empresa
                            vinculada no CRM.
                          </div>
                          <div>
                            {gerencial.motivosPerdaFunil2.total > 0
                            ...
                            Recusas em Análise e Cotação não exigem motivo — são
                            decisão de compliance da própria seguradora, não do
                            time.
                          </div>
                          {gerencial.qualidade.saiuFunil1SemHoraFim > 0 && (
                            <div>
                              {gerencial.qualidade.saiuFunil1SemHoraFim} card(s)
                              já recusado(s)/aprovado(s) em Análise e Cotação
                              sem HORA FIM da cotação registrada.
                            </div>
                          )}
                          {gerencial.qualidade.semResponsavelCotacao > 0 && (
                            <div>
                              {gerencial.qualidade.semResponsavelCotacao}{" "}
                              card(s) sem Responsável(is) pela Cotação
                              preenchido.
                            </div>
                          )}
                          {gerencial.qualidade.semResponsavelNegociacao > 0 && (
                            <div>
                              {gerencial.qualidade.semResponsavelNegociacao}{" "}
                              card(s) que já entraram em &quot;Contrato
                              Recebido&quot; sem Responsável(is) pela Negociação
                              preenchido.
                            </div>
                          )}
                          {gerencial.qualidade.semResponsavelEfetivacao > 0 && (
                            <div>
                              {gerencial.qualidade.semResponsavelEfetivacao}{" "}
                              card(s) com Data de Efetivação preenchida sem
                              Responsável pela Efetivação.
                            </div>
                          )}
                          {gerencial.qualidade.naoAdministrados > 0 && (
                            <div style={{ color: "var(--ink-faint)" }}>
                              {gerencial.qualidade.naoAdministrados} card(s)
                              marcados como &quot;Não Administrada&quot;
                              (proprietário direto, sem imobiliária) —
                              informativo, não é alerta.
                            </div>
                          )}
                        </div>
                      </div>
              ...
              <footer className={styles.footer}>
                Fonte: Bitrix24, SPA &quot;Seguro Fiança&quot; (entityTypeId{" "}
                {ENTITY_TYPE_ID}), funis &quot;Análise e Cotação&quot; (
                {CATEGORIA_ANALISE}) e &quot;Negociação e Contrato&quot; (
                {CATEGORIA_NEGOCIACAO}
                ), via webhook de leitura. {gerencial.totalMovimentacoes}{" "}
                movimentações de etapa registradas no histórico nativo do CRM.
                &quot;Produtividade&quot; conta o responsável ATUAL de cada card
                — o CRM não guarda histórico de troca de responsável via API, só
                de troca de etapa. &quot;Valores trabalhados&quot; mostram
                volume de trabalho, não receita da corretora; receita real só
                existe a partir do prêmio líquido e comissão de contratos
                convertidos.
              </footer>
            </AbasProvider>
          )}
