...
## Planilha de conferência — Ficha Fiança e Capitalização

Direção **oposta** às integrações acima: aqui é a Plataforma O2 (landing pages `/ficha-fianca` e `/capitalizacao`) que empurra uma linha pra dentro de uma planilha do Google, a cada envio — não a planilha que aciona a Plataforma O2.
...
- **Uma única planilha, uma única implantação do Apps Script, atendendo os dois formulários** — cada envio diz de qual formulário é (campo `formulario: "ficha_fianca" | "capitalizacao"` no corpo) e cai na aba certa ("Respostas" ou "Capitalização"), criada automaticamente no primeiro envio. Pra adicionar um formulário novo no futuro, só acrescentar uma entrada em `CONFIG` no `.gs` — não precisa de planilha nem implantação novas.
- Código da planilha: `ficha-fianca-planilha.gs`.
- Lado Next.js: `src/lib/integracoes/planilhaSeguroFianca.ts` (Ficha Fiança) e `src/lib/integracoes/planilhaCapitalizacao.ts` (Capitalização), ambos chamados depois que o card já foi criado no Bitrix — falha na planilha nunca derruba o envio.
