...
## Planilha de conferência — Ficha Fiança

Direção **oposta** às integrações acima: aqui é a Plataforma O2 (landing page `/ficha-fianca`) que empurra uma linha pra dentro de uma planilha do Google, a cada envio — não a planilha que aciona a Plataforma O2.

- O Google Forms "Ficha Fiança 5G" (e a planilha de respostas dele) **continua ativo e intocado** — essa planilha nova é só da landing page nova, separada de propósito (ver decisão registrada no código: escrever na planilha do Forms original arriscaria descolar as colunas se alguém editar o Forms no futuro).
- Código da planilha: `ficha-fianca-planilha.gs`.
- Lado Next.js: `src/lib/integracoes/planilhaSeguroFianca.ts`, chamado depois que o card já foi criado no Bitrix — falha na planilha nunca derruba o envio.
- Variáveis de ambiente (local/Vercel): `GOOGLE_SHEETS_FICHA_FIANCA_URL` (URL do App da Web) e `GOOGLE_SHEETS_FICHA_FIANCA_SECRET` (mesmo texto do `TOKEN_ESPERADO` no `.gs`).

### Configuração (uma vez só)

1. Crie uma planilha nova em branco no Google Sheets (ex: "Ficha Fiança — Conferência Landing Page").
...
4. Implantar > Nova implantação > tipo "App da Web". Executar como "Eu", quem pode acessar "Qualquer pessoa". Autorize quando pedir — **precisa ser feito no navegador normal, não funciona em navegador automatizado** (mesma limitação já documentada na integração de Capitalização).
5. Copie a URL do App da Web gerada.
6. Cadastre `GOOGLE_SHEETS_FICHA_FIANCA_URL` (a URL) e `GOOGLE_SHEETS_FICHA_FIANCA_SECRET` (o mesmo token do passo 3) no `.env.local` e na Vercel (Production).
7. Se editar o `.gs` depois, use "Gerenciar implantações > Editar > nova versão" em vez de "Nova implantação", pra manter a mesma URL (senão as variáveis de ambiente precisam ser atualizadas de novo).
